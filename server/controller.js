...
module.exports = {
    register: (req, res) => {
    const {user_login_name, user_password, user_photo} = req.body;
    req.app.get('db').register_user([user_login_name, user_password, user_photo])
      .then(() => {
        res.status(200).send();
      })
...
login: (req, res) => {
  ...
  req.app.get('db').login_user([user_login_name])
  .then((response) => {
    if (response[0]) {
      if (response[0].user_password === user_password) {
